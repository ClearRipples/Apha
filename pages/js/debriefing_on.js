 ...
 */
  ...
  * #1. 扫描或从列表界面进入插件
  * #2. 一次只能汇报一张任务单
  * #3. 在汇报详情界面中，再次进行扫描单据(不同任务单)的情况下，提示单据是否进行保存,保存则完成汇报，
  		否则提示退出并生成新的汇报单
  * #4. 扫描重复的汇报单时，除数量不同外，其他都一样的情况进行数量合并数量累加（后台进行处理）
  		若进行班组，操作工修改操作后，则表示不同分录，需要进行新增（后台进行处理）返回新的key
...
(function(window, document, undefined) {
	...
	//////////////////////////////////
	//在初始化时进行数据的绑定
	DebriefingProto.BindData = function(data){
